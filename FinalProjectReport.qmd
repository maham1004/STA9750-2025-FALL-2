...
# **To what extent do socioeconomic conditions predict variation in subway crime rates?**
...
# Download ACS 2023 median household income by ZCTA 
...
#  Get ZCTA polygons and detect the ID column
...
#  Convert crime_clean to sf points using longitude and latitude
crime_sf <- st_as_sf(
  crime_clean,
...
#  Spatial join: assign ZCTA ID to each crime point
...
# Build interactive detailed crime-level datatable
...
# fit linear model
lm_fit <- lm(avg_crime_per_100k_rides ~ Income, data = zip_summary_trimmed)

# extract R-squared
r2_value <- summary(lm_fit)$r.squared

# plot with regression line and R-squared
...
This analysis finds a clear and statistically significant negative relationship between neighborhood income levels and subway crime risk in New York City. ZIP codes with higher median household income tend to experience lower subway crime rates per 100,000 rides, even after accounting for differences in ridership. While the analysis does not establish causality, it shows that variation in subway crime risk is closely aligned with neighborhood socioeconomic conditions rather than being evenly distributed across the system.

These findings suggest that subway crime is shaped by more than passenger volume alone. Areas with similar ridership levels can face very different levels of risk, indicating that local context matters for transit safety outcomes. From a policy perspective, this implies that strategies focused only on high-traffic stations may miss neighborhoods where riders face disproportionately higher exposure to crime. Incorporating socioeconomic context into safety planning could help identify locations where investments in enforcement, station design, or social services may have the greatest marginal impact, rather than applying uniform approaches across the system.

A key limitation of this analysis is the imperfect measurement of ridership. Subway ridership data is not available at the ZIP code level, so station ridership had to be spatially assigned to ZIP codes based on station locations. This approach does not fully capture how riders move across neighborhoods or where trips actually begin and end, which may introduce measurement error in the crime rates and provide a less than accurate depiction of the income levels of passengers at any given station. 
